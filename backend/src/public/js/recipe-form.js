...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (path === '/create-recipe') {
        ...
        isEditMode = true;
        const match = path.match(/\/edit-recipe\/(\d+)/);
        if (match) {
            recipeId = match[1];
...
async function loadRecipeData(id) {
    try {
        ...
        document.getElementById('dietary_preferences').value = recipe.dietary_preferences 
            ? recipe.dietary_preferences.join(', ') 
            : '';
        ...
        if (error.response?.status === 401) {
            ...
            alert('Error loading recipe: ' + (error.response?.data?.message || error.message));
...
async function handleFormSubmit(e) {
    ...
    try {
        ...
        formData.append('prep_time', document.getElementById('prep_time').value || 0);
        formData.append('cook_time', document.getElementById('cook_time').value || 0);
        formData.append('servings', document.getElementById('servings').value || 1);
        formData.append('difficulty', document.getElementById('difficulty').value);
        
        const dietary = document.getElementById('dietary_preferences').value
            .split(',')
            .map(d => d.trim())
            .filter(d => d);
