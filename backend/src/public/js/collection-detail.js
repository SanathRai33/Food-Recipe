...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const match = path.match(/\/collection\/(\d+)/);
    if (match) {
        collectionId = match[1];
        loadCollectionDetail(collectionId);
        setupAddRecipeListener();
    } else {
        document.getElementById('collectionDetail').innerHTML = '<div class="empty-message">Invalid collection ID</div>';
    }
});
...
function displayCollectionRecipes(recipes) {
    ...
    if (!recipes || recipes.length === 0) {
        container.innerHTML = '<div class="empty-message">No recipes in this collection yet.</div>';
        return;
    }

    const user = JSON.parse(localStorage.getItem('user') || '{}');
    const isOwner = document.querySelector('.collection-actions .btn-edit') !== null;
...
function setupAddRecipeListener() {
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            ...
            if (recipeId) {
                addRecipeToCollection(parseInt(recipeId));
            } else {
...
async function addRecipeToCollection(recipeId) {
    ...
    try {
        const token = localStorage.getItem('token');
        await axios.post(`/api/collections/${collectionId}/recipes`, 
            { recipe_id: recipeId },
            { headers: { 'Authorization': `Bearer ${token}` } }
        );
...
async function removeRecipeFromCollection(recipeId) {
    ...
    try {
        ...
        alert('Error removing recipe: ' + (error.response?.data?.message || error.message));
    }
}
