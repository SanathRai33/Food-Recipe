...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const match = path.match(/\/recipe\/([^/]+)\/ratings/);
  if (match) {
    recipeId = match[1];
    ...
    document.querySelector(".ratings-content").innerHTML =
      '<div class="empty-ratings"><p>Invalid recipe</p></div>';
  }
});
...
async function loadRatings(id) {
  try {
    // Load all ratings
    const ratingsResponse = await axios.get(`/api/ratings/recipe/${id}`);
    ...
    if (token) {
      try {
        const userResponse = await axios.get(`/api/ratings/me/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
...
function displayUserRating(rating) {
  ...
  if (rating) {
    section.innerHTML = `
            ...
            <button id="submitRatingBtn">Update Rating</button>
    ...
    section.innerHTML = `
            ...
            <div class="rating-stars-input">
                ${[1, 2, 3, 4, 5]
                  .map(
                    (num) => `
                    <button class="star-btn" data-value="${num}">★</button>
                `,
                  )
                  .join("")}
            </div>
...
function displayAllRatings(ratings) {
  ...
  if (!ratings || ratings.length === 0) {
    container.innerHTML =
      '<div class="empty-ratings"><p>No ratings yet</p></div>';
    return;
  }
