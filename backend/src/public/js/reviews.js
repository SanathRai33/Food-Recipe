...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const match = path.match(/\/recipe\/([^/]+)\/reviews/);
  if (match) {
    recipeId = match[1];
    ...
    document.querySelector(".reviews-content").innerHTML =
      '<div class="empty-reviews"><p>Invalid recipe</p></div>';
  }
});
  ...
  try {
    ...
    document.getElementById("recipeInfo").innerHTML = `
            ...
            <div class="recipe-meta">
                <span>👤 By: ${recipe.User?.username || "Unknown"}</span>
...
async function loadReviews(id, page = currentPage) {
  ...
  container.innerHTML =
    '<div class="empty-reviews"><p>Loading reviews...</p></div>';

  try {
    const token = localStorage.getItem("token");
    const response = await axios.get(
      `/api/reviews/recipe/${id}?page=${page}&limit=${limit}`,
      {
    ...
    if (data.reviews.length === 0) {
      container.innerHTML =
        '<div class="empty-reviews"><p>No reviews yet. Be the first to review this recipe!</p></div>';
...
function displayReviews(container, reviews) {
      ...
      return `
            <div class="review-item" id="review-${review.id}">
                <div class="review-header">
                    ...
                    <span class="review-date">${new Date(review.createdAt).toLocaleDateString()} ${new Date(review.createdAt).toLocaleTimeString()}</span>
                </div>
                ${
                  isEditing
                    ? `
                    ...
                    <div class="review-actions">
                        <button class="btn-save-review" data-id="${review.id}">💾 Save</button>
                ...
                `
                    ...
                    ${
                      isOwner
                        ? `
                        <div class="review-actions">
                            <button class="btn-edit-review" data-id="${review.id}">✏️ Edit</button>
                            <button class="btn-delete-review" data-id="${review.id}">🗑️ Delete</button>
                        </div>
                    `
                        : ""
                    }
  ...
  container.querySelectorAll(".btn-save-review").forEach((btn) => {
    btn.addEventListener("click", () => {
      saveReview(btn.dataset.id);
    });
  });

  // Cancel Edit
  container.querySelectorAll(".btn-cancel-review").forEach((btn) => {
...
async function saveReview(reviewId) {
  const content = document
    .getElementById(`edit-content-${reviewId}`)
    .value.trim();
